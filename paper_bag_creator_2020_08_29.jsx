            ...
            ui_main_group2.alignChildren = 'right'; 
              ...
              var ui_group8 = ui_main_group2.add ("group");
                ui_group8.alignment = "left"; 
                 var ui_square_bottom = ui_group8.add ("checkbox", undefined, "Разметка под крадраное дно");
                ui_square_bottom.value = false;   
               // ui_square_bottom.enabled = false;
...
ui_bag_width.onChanging = ui_bag_height.onChanging = ui_bag_deep.onChanging =ui_bag_side_flap.onChanging = ui_bag_bottom.onChanging =ui_bag_top_flap.onChanging  = function () {
    ...
    statictext_ui_alert_button.enabled =ui_button_run.enabled = valid_ui_bag_width && valid_ui_bag_height&& valid_ui_bag_deep && valid_ui_bag_side_flap && valid_ui_bag_bottom && valid_ui_bag_top_flap;
...
ui_bag_deep.onChanging = function () {
    ui_bag_bottom.text = Number (ui_bag_deep.text/2 + bottom_flap);
    }
...
ui_right_bind.onClick= function(){
  ...
  if (square_bottom== true){
        square_bottom.value = true;
        } else {
             square_bottom.value = false;
            }
        
ui_square_bottom.onClick= function(){
    square_bottom = true;
            } 
ui_square_bottom.onClick= function(){
      lsquare_bottom= false;
            }  
 ...
 ui_button_PDF.onClick = function () {
     key_create_PDF= true;
                w.close ();
                                    ...
                                    // create_template (width, height, deep, side_flap, bottom, top_flap, left_bind, square_bottom)
   if ( w.show ()==1 ) create_template (ui_bag_width.text, ui_bag_height.text, ui_bag_deep.text, ui_bag_bottom.text, ui_bag_top_flap.text, ui_left_bind.value, ui_square_bottom.value);
   if (key_create_PDF==true) {create_template (ui_bag_width.text, ui_bag_height.text, ui_bag_deep.text, ui_bag_bottom.text, ui_bag_top_flap.text, ui_left_bind.value, ui_square_bottom.value); create_pdf (width, height, deep, path_PDF_folder, name_PDF);}
       

}

function  create_template ( width, height, deep, bottom, top_flap, left_bind, square_bottom){
...
if (square_bottom == true){
       myDoc.pages.item(0).graphicLines.add({strokeColor:"Marks", strokeWeight: mark_thickness, geometricBounds:[point_i_y ,  point_i_x , point_j_y , point_j_x ]});
       myDoc.pages.item(0).graphicLines.add({strokeColor:"Marks", strokeWeight: mark_thickness, geometricBounds:[point_j_y ,  point_j_x , point_m_y , point_m_x + 12]});
       myDoc.pages.item(0).graphicLines.add({strokeColor:"Marks", strokeWeight: mark_thickness, geometricBounds:[point_m_y ,  point_m_x + 12 , point_o_y , point_o_x - 12]});
       myDoc.pages.item(0).graphicLines.add({strokeColor:"Marks", strokeWeight: mark_thickness, geometricBounds:[point_o_y , (point_o_x - 12) , point_k_y , point_k_x ], flip: 1752134266});
       myDoc.pages.item(0).graphicLines.add({strokeColor:"Marks", strokeWeight: mark_thickness, geometricBounds:[point_k_y , point_k_x  , point_o_y , point_o_x + 12]});
       myDoc.pages.item(0).graphicLines.add({strokeColor:"Marks", strokeWeight: mark_thickness, geometricBounds:[point_o_y , point_o_x + 12 , point_p_y , point_p_x - 12]});
       myDoc.pages.item(0).graphicLines.add({strokeColor:"Marks", strokeWeight: mark_thickness, geometricBounds:[point_l_y , point_l_x, point_p_y , point_p_x - 12], flip: 1752134266});
